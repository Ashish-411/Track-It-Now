...
function DeliveryControls() {
    const { agentParcel, agentAssignment, clearActiveDelivery } = useNotification();
    const { user } = useAuth();
    const [currentStatus, setCurrentStatus] = useState(() =>{
      try{
        const stored = localStorage.getItem("deliveryStatus");
 ...
 const updateStatus = async (status) => {
        setLoading(true);
        try {
            await api.post(`/api/parcel/status/update?parcel_id=${agentParcel?.id}&status=${status}`);
            setCurrentStatus(status);
            localStorage.setItem("deliveryStatus",status);
            console.log("Status updated:", status);

            if (status === "delivered") {
                await api.post(`/api/parcel/delivery_completed?parcel_id=${agentParcel?.id}&tracking_code=${agentAssignment?.tracking_code}`)
                clearActiveDelivery();
 ...
 const isDone = (requires) => {
        ...
        return order.indexOf(currentStatus) > order.indexOf(requires);
    };
        ...
        <div style={{
            ...
            <div style={{ marginBottom: 14, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <span style={{
                    ...
                    background:    currentStatus === "delivered" ? "rgba(0,229,153,0.12)"
                                 : currentStatus === "in_transit" ? "rgba(245,158,11,0.12)"
                                 : currentStatus === "picked_up"  ? "rgba(79,138,255,0.12)"
                                 : "rgba(245,158,11,0.12)",
                    border:        currentStatus === "delivered" ? "1px solid rgba(0,229,153,0.25)"
                                 : currentStatus === "in_transit" ? "1px solid rgba(245,158,11,0.25)"
                                 : currentStatus === "picked_up"  ? "1px solid rgba(79,138,255,0.25)"
                                 : "1px solid rgba(245,158,11,0.25)",
                    color:         currentStatus === "delivered" ? "#00e599"
                                 : currentStatus === "in_transit" ? "#f59e0b"
                                 : currentStatus === "picked_up"  ? "#7aaaff"
                                 : "#f59e0b",
                }}>
                    <span style={{
            ...
            <div style={{ display: "flex", gap: 8 }}>
                {steps.map(({ label, status, requires, icon }) => {
                    const isActive = currentStatus === requires;
